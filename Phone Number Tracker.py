...
def process_number(number):
    try:
        ...
        """Display a message indicating the tracking attempt. We'll also format the parsed number to the international format"""
        print(f"{Fore.GREEN}[+] Attempting to track location of "f"{phonenumbers.format_number(parsed_number,phonenumbers.PhoneNumberFormat.INTERNATIONAL)}...")
        ...
        """Get the service provider (carrier) and display it if available. Some businesses and organisations do not use public service providers. So you may not see the carrier in that case."""
...
def get_approx_coordinates():
    ...
    try:
        ...
        coder = OpenCageGeocode("Insert your OpenCageGeocode API key here")
        query = location
        # Perform a geocoding query to obtain results.
        results = coder.geocode(query)
        # Extract latitude and longitude from the geocoding results. These are the coordinates of the phone number's location
        ...
        """Handle exception by printing an error message and exiting the script. This will prevent the program from crashing"""
        print(f"{Fore.RED}[-] Could not ge tthe location of this number. Please specify a valid phone number or check your internet connection.")
        sys.exit()

# This function basically removes unwanted characters unwanted from the Phone Number such as white spaces
...
def draw_map():
    try:
        # Create a Folium map centerd around the latitude and klongidude of the number's coordinates.
        ...
        we'll basically save wach map with the number of the owner for easy identification. """
    ...
    # Handle the "NameError' exception, which can occur if the 'latitude' and 'longitude' variables are not defined
    except NameError:
...
def cli_argument():
    ...
    parser.add_argument("-p","--phone",dest = "phone_number", type = str, help = "Phone number to track, Please include the country code whne specifying the number.", required = True, nargs = "+")
...
# Parse command-line arguments using the 'cli-argument' function
args = cli_argument()
